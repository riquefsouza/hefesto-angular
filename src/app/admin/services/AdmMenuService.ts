...
export class AdmMenuService {
    ...
    */
    ...
    public async report(obj: ReportParamForm): Promise<any> {
        ...
        .pipe(
            tap(
              data => FileSaver.saveAs(data, 'AdmMenu.' + obj.reportType.toLowerCase()),
              error => this.errorService.log(`report AdmMenu: ${error}`)
            ),
            catchError(this.errorService.handleError<any>('report AdmMenu'))
          )
        .toPromise();
