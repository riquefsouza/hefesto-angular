...
export class AdmUserService {
    ...
    */
    ...
    public async report(obj: ReportParamForm): Promise<any> {
        ...
        .pipe(
            tap(
              data => FileSaver.saveAs(data, 'AdmUser.' + obj.reportType.toLowerCase()),
              error => this.errorService.log(`report AdmUser: ${error}`)
            ),
            catchError(this.errorService.handleError<any>('report AdmUser'))
          )
        .toPromise();
